...
## Load cubes (generated using _getdata.py script)
...
plt.tight_layout()
plt.savefig('Nepal_tas_djf_ts.png')
...
future_maps(cube_mid, cube_end, np.linspace(-3, 3, 20), reg['Nepal'], axs)
for ax in axs:
    ax.plot([85, 85], [25, 30], 'k')

plt.savefig('Nepal_tas_djf_map.png')
...
plt.tight_layout()
plt.savefig('Nepal_tas_jja_ts.png')
...
cube_end = x_end - x_now
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(6,3))
future_maps(cube_mid, cube_end, np.linspace(-4, 4, 20), reg['Nepal'], axs)
for ax in axs:
    ax.plot([85, 85], [25, 30], 'k')

plt.savefig('Nepal_tas_jja_map.png')
...
plt.tight_layout()
plt.savefig('Nepal_tasmax_jja_ts.png')
...
plt.savefig('Nepal_tasmax_jja_map.png')
...
fig, axs = plt.subplots(1, 3, figsize=(8, 3))
# change plot size (so no resizing at end)
nep_list = get_region_ts('Nepal', pr)
plot_spagetti(nep_list, axs[0])
axs[0].set_ylabel('Pr, mm')
axs[0].set_title('Nepal')
nep_list = get_region_ts('Nepal_west', pr)
plot_spagetti(nep_list, axs[1])
axs[1].set_title('West')
nep_list = get_region_ts('Nepal_east', pr)
plot_spagetti(nep_list, axs[2])
axs[2].set_title('East')
for ax in axs:
    ax.set_ylim([-300, 600])
...
plt.tight_layout()
plt.savefig('Nepal_pr_jja_ts.png')
...
cube_end = x_end - x_now
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(6,3))
future_maps(cube_mid, cube_end, np.linspace(-300, 300, 20), reg['Nepal'], axs)
for ax in axs:
    ax.plot([85, 85], [25, 30], 'k')

plt.savefig('Nepal_pr_jja_map.png')
...
### PRMAX JJA
# Timeseries
tas_djf = iris.load('/user/work/hh21501/wash/prmax_ssp242_jja.nc')
pr = []
for each in tas_djf:
...
plt.tight_layout()
plt.savefig('Nepal_prmax_jja_ts.png')
...
cube_end = x_end - x_now
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(6,3))
future_maps(cube_mid, cube_end, np.linspace(-10, 10, 20), reg['Nepal'], axs)
for ax in axs:
    ax.plot([85, 85], [25, 30], 'k')

plt.savefig('Nepal_prmax_jja_map.png')
