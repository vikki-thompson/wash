...
## Define regions
reg = {'Nepal': [23, 33, 78, 91], 'Nepal_west':[23, 33, 78, 85],'Nepal_east':[23, 33, 85, 91], 'Ethiopia':[3, 17, 30, 50], 'Ethiopia_lo':[3, 17, 40, 50], 'Ethiopia_hi':[3, 17, 30, 40], 'Uganda':[-4, 6, 28, 35], 'Uganda_north':[1, 6, 28, 35], 'Uganda_south':[-4, 1, 28, 35], 'Senegal':[11, 17, 341, 351], 'Sen_south':[11, 14.5, 341, 351], 'Sen_north':[14, 17, 341, 351]} # S, N, W, E
...
def future_maps(cube_mid, cube_end, crange, region, axs):
    ...
    for each in axs:
        ...
        each.set_xlim([region[2]-1-360, region[3]+1-360]) 
        # -360 added for Senegal
...
nep_list = get_region_ts('Senegal', tas_djf)
plot_spagetti(nep_list, axs[0])
axs[0].set_ylabel('Temp, *C')
axs[0].set_title('Ethiopia')
north_list = get_region_ts('Sen_south', tas_djf)
plot_spagetti(north_list, axs[1])
axs[1].set_title('South')
south_list = get_region_ts('Sen_north', tas_djf)
plot_spagetti(south_list, axs[2])
axs[2].set_title('North')
for ax in axs:
    ax.set_ylim([-.5, 3.5])

plt.tight_layout()
plt.savefig('Senegal_tas_djf_ts.png')
...
future_maps(cube_mid, cube_end, np.linspace(-2, 2, 20), reg['Senegal'], axs)
for ax in axs:
    ax.plot([342.8-360, 348-360], [14.2, 14.2], 'k')

plt.savefig('Senegal_tas_djf_map.png')
...
plt.tight_layout()
plt.savefig('Senegal_tas_jja_ts.png')
...
future_maps(cube_mid, cube_end, np.linspace(-2, 2, 20), reg['Senegal'], axs)
for ax in axs:
    ax.plot([40, 40], [3, 17], 'k')

plt.savefig('Senegal_tas_jja_map.png')
...
plt.tight_layout()
plt.savefig('Senegal_tasmax_jja_ts.png')
...
future_maps(cube_mid, cube_end, np.linspace(-2, 2, 20), reg['Senegal'], axs)
for ax in axs:
    ax.plot([341, 390], [14.5, 14.5], 'k')

plt.savefig('Senegal_tasmax_jja_map.png')
...
fig, axs = plt.subplots(1, 3, figsize=(8, 3))
# change plot size (so no resizing at end)
nep_list = get_region_ts('Senegal', pr)
plot_spagetti(nep_list, axs[0])
axs[0].set_ylabel('Pr, mm')
axs[0].set_title('Senegal')
nep_list = get_region_ts('Sen_south', pr)
plot_spagetti(nep_list, axs[1])
axs[1].set_title('South')
nep_list = get_region_ts('Sen_north', pr)
...
plt.tight_layout()
plt.savefig('Senegal_pr_jja_ts.png')
...
cube_end = x_end - x_now
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(6,3))
future_maps(cube_mid, cube_end, np.linspace(-120, 120, 20), reg['Senegal'], axs)
for ax in axs:
    ax.plot([29, 35], [1, 1], 'k')

plt.savefig('Senegal_pr_jja_map.png')
...
axs[0].set_ylabel('Pr, mm')
axs[0].set_title('Senegal')
nep_list = get_region_ts('Sen_southi', pr)
plot_spagetti(nep_list, axs[1])
axs[1].set_title('South')
nep_list = get_region_ts('Sen_north', pr)
plot_spagetti(nep_list, axs[2])
...
plt.tight_layout()
plt.savefig('Senegal_pr2_jja_ts.png')
...
cube_end = x_end - x_now
fig, axs = plt.subplots(1, 2, subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(6,3))
future_maps(cube_mid, cube_end, np.linspace(-10, 10, 20), reg['Senegal'], axs)
for ax in axs:
    ax.plot([40, 40], [3, 17], 'k')

plt.savefig('Senegal_pr2_jja_map.png')
